...
public class DownloadImages{
    ...
    private static String[] threadCount2 = new String[200];
    private static String boardChoice = "";
    private static boolean isRefreshed = false;
    private static final String[] boardList = {"/a/", "/b/", "/c/", "/d/", "/e/","/g/", "/gif/", "/h/", "/hr/",
	    "/k/", "/m/", "/o/", "/p/", "/r/", "/s/", "/t/", "/u/", "/v/", "/vg/",
	    "/vr/", "/w/", "/wg/", "/i/", "/ic/", "/r9k/", "/s4s/","/cm/","/hm/","/lgbt/",
	    "/y/","/3/","/adv/","/an/","/asp/","/biz/","/ck/","/co/","/diy/",
	    "/fa/","/fit/","/gd/","/hc/","/int/","/jp/","/lit/","/mlp/","/toy/","/trv/",
	    "/tv/","/vp/","/wsg/","/x/"};
    private static String[][] threadList = new String[boardList.length][200];
    ...
    public static void main(String[] args) {
	    ...
	    //stores the name of all threads (ex: /a/) in threadList
	    for(int i = 0; i < boardList.length; i++){
		    //threadList[i][i] = "";
		    threadList[i][0] = "http://boards.4chan.org"+boardList[i];
		   ...
		   for(int i = 0; i < threadCount; i++){
		        try {
		     	  ...
		     	  System.out.println(threadCount2[i]);
		            Document doc = Jsoup.connect(threadCount2[i]).get(); 
		            
		            //Get all elements with img tag ,
		            Elements img = doc.getElementsByClass("fileThumb");
    ...
    public static void threads(String board){
	   ...
	   for(int j = 0; j < 10; j++){
	    ...
	    try {
		    	//connects to board
	            Document doc2 = Jsoup.connect("http://boards.4chan.org"+boardChoice).get(); 
	            setText(boardChoice);
	            //Get all elements with img tag ,
	            ...
	            for (Element el : img2) {
	                ...
	                threadList[boardIndex][q] = "http://boards.4chan.org"+boardChoice+src2;
	                
	                setText(threadList[boardIndex][q]);
 				q++;
	            }
